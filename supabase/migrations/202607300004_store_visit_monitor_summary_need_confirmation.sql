-- Add need_confirmation_count to store-visit-monitor summaries.
-- 口径对齐 H5：resolveCandidatePriceHandling → MANUAL_CONFIRMATION_REQUIRED
-- （未 approved/rejected，且 NEED_REVIEW / REVIEW_REQUIRED / INSUFFICIENT_BENCHMARK /
--  FAILED 且 attempt>=3；排除 deleted/replaced/reanalyzed）。

drop function if exists public.store_visit_monitor_promoter_summary(
  date, date, text, text, text, text, uuid[], integer, integer
);
drop function if exists public.store_visit_monitor_store_summary(
  date, date, text, text, text, text, uuid[], integer, integer
);
...
returns table (
  ...
  need_confirmation_count integer,
...
begin
  ...
  quality as (
    select
      ...
      count(*) filter (
        where coalesce(c.h5_lifecycle_status, '') not in ('deleted', 'replaced', 'reanalyzed')
          and c.status not in ('approved', 'rejected')
          and (
            c.review_decision = 'NEED_REVIEW'
            or c.quality_gate_status in ('REVIEW_REQUIRED', 'INSUFFICIENT_BENCHMARK')
            or (
              c.quality_gate_status = 'FAILED'
              and coalesce(c.quality_gate_attempt_count, 0) >= 3
            )
          )
      )::integer as need_confirmation_count
  ...
  aggregated as (
    select
      ...
      coalesce(sum(q.need_confirmation_count), 0)::integer as need_confirmation_count_value
  ...
  ranked as (
    select
      ...
      a.need_confirmation_count_value,
      case
  ...
  select
    ...
    r.need_confirmation_count_value,
    r.pass_rate_value,
    r.total_count_value
  from ranked r
  order by
...
comment on function public.store_visit_monitor_promoter_summary(date, date, text, text, text, text, uuid[], integer, integer) is
  'Paginated promoter summary for store-visit-monitor, including H5 need-confirmation counts.';

comment on function public.store_visit_monitor_store_summary(date, date, text, text, text, text, uuid[], integer, integer) is
  'Paginated store summary for store-visit-monitor, including H5 need-confirmation counts.';
