...
with ranked_makuku_duplicates as (
  select
    id,
    row_number() over (
      partition by source, captured_at, offline_store_id, sku_master_id
      order by created_at asc, id asc
    ) as rank
  from public.price_snapshots
  where source like 'excel_import:%'
...
create unique index if not exists uniq_price_snapshots_excel_makuku
  on public.price_snapshots(source, captured_at, offline_store_id, sku_master_id)
  where source like 'excel_import:%'
    and sku_master_id is not null
    and competitor_product_id is null;
...
begin
  ...
  upserted as (
    ...
    where source like 'excel_import:%'
      and sku_master_id is not null
      and competitor_product_id is null
    on conflict (source, captured_at, offline_store_id, sku_master_id)
      where source like 'excel_import:%'
        and sku_master_id is not null
        and competitor_product_id is null
    do update set
