...
export const PRICE_IMAGE_PROMPT_VERSION = "price-evidence-v1.1-majority-gate";
...
const STORE_VISIT_PRICE_IMAGE_PROMPT = [
  ...
  "WORKFLOW: Step 1 evaluate image quality before extraction. If status is retake_required, rows must be [] and extraction must stop. If status is pass, extract all reliable readable price evidence groups and leave unreliable groups out or mark them with low confidence and PARSE_RISK.",
  "PHOTO QUALITY PASS GATE: pass when the main intended price-tag or price-board capture area has a clear majority of target rows/tags readable enough to verify product-price evidence. For that majority, price digits must be readable, product/SKU or row identity must be readable, and product-to-price binding must be reliable.",
  "A clear majority means most target rows/tags in the foreground intended capture area can be reliably read and bound. It does not require every row/tag to be readable. Ignore unrelated tiny background tags, cropped neighboring boards, or non-target shelf labels when judging the majority.",
  "Do not pass a wide shelf overview, long side-angle shelf shot, distant multi-section shelf photo, or handwritten board just because one or two front price tags or isolated rows are readable. If only a minority of target tags/rows in the intended capture area are readable, or if most are distorted by perspective, blocked, or cannot be matched to products/SKUs, use retake_required.",
  "Use retake_required when price digits are unclear for most target rows/tags, the photo angle prevents reliable row/tag binding for most target rows/tags, price tags are obstructed, the image is a wide shelf overview with only a few readable tags, or product-price relationship cannot be confirmed for a clear majority of the intended capture area. photo_quality.reasons may contain only: price_unclear, angled_affects_reading, price_obstructed. If status is pass, reasons=[].",
