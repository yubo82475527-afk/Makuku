...
const STORE_VISIT_PRICE_IMAGE_PROMPT = [
  ...
  "WORKFLOW: Step 1 evaluate image quality before extraction. If status is retake_required, rows must be [] and extraction must stop. If status is pass, extract all readable price evidence groups.",
  "PHOTO QUALITY PASS GATE: pass only when the intended price-tag or price-board capture area is readable enough to verify the target product-price evidence. Price digits must be readable, the product/SKU or row identity must be readable, and product-to-price binding must be reliable for the captured target area.",
  "Do not pass a wide shelf overview, long side-angle shelf shot, or distant multi-section shelf photo just because one or two front price tags are readable. If most target tags/rows in the intended capture area are unreadable, distorted by perspective, blocked, or cannot be matched to products/SKUs, use retake_required.",
  "Use retake_required when price digits are unclear, the photo is too angled for reliable row/tag binding, price tags are obstructed, the image is a wide shelf overview with only a few readable tags, or product-price relationship cannot be confirmed for the intended capture area. photo_quality.reasons may contain only: price_unclear, angled_affects_reading, price_obstructed. If status is pass, reasons=[].",
