...
const STORE_VISIT_PRICE_IMAGE_PROMPT = [
  ...
  "For each price-board table row, return row-level evidence text from the SAME row: piece_count_text from the Pcs cell, list_price_text from HARGA NORMAL/PACK, package_price_text from the visible whole-package selling price, net_price_text from HARGA PROMO/PACK, and visible_price_per_piece_text from the final payable HARGA/PCS.",
  "If a row has both NORMAL and PROMO price groups, net_price_text and visible_price_per_piece_text must both come from the PROMO group. Do not mix promo package price with normal HARGA/PCS.",
  "If no PROMO group is visible for that row, visible_price_per_piece_text may come from the NORMAL HARGA/PCS column.",
  ...
  "visible_price_per_piece_text is final analysis piece-price evidence. It must not be written into list_price_idr, package_price_idr, or net_price_idr.",
