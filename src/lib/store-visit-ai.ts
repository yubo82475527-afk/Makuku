...
const STORE_VISIT_PRICE_IMAGE_PROMPT = [
  ...
  "WORKFLOW: First evaluate photo quality. If photo_quality.status is retake_required, rows must be [] and extraction must stop. If photo_quality.status is pass, extract reliable visible price evidence row by row.",
  ...
  "PHOTO QUALITY: First identify the primary price board, price tag, or promotion card intended to be captured. Evaluate quality by that primary visual section, not by the entire image.",
  "The photo passes only when the primary visual section is captured at a usable size and orientation, a clear majority of visible rows are readable, that majority has readable row anchors and price cells, and same-row product-to-price binding is reliable for that majority.",
