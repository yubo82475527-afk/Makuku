...
export const mobileCopy: Record<Locale, MobileCopy> = {
  en: {
    ...
    newVisitHint: "Upload Makuku and competitor price-tag photos for analysis. Store Display photos are stored only in 1.0.",
  ...
  zh: {
    ...
    newVisitHint: "上传 Makuku 和竞品价格标签用于分析。门店陈列照片在 1.0 只做存档，不进入 AI 解析。",
    storeInformation: "门店信息",
    storeName: "门店名称",
    region: "区域",
    promoter: "巡店员",
    shelfPhotos: "巡店照片",
    makukuShelfPhotos: "Makuku 价格标签",
    competitorShelfPhotos: "竞品价格标签",
    storefrontPhotos: "门店陈列",
    ...
    submitting: "提交中...",
    completeStoreInfo: "请完善门店信息。",
    uploadAtLeastOnePhoto: "请至少上传一张价格标签照片。",
    uploadMakukuShelfRequired: "请至少上传一张 Makuku 价格标签照片。",
    submitFailed: "提交巡店失败。",
...
export function mobileAnalysisStatusLabel(locale: Locale, status: StoreVisitAnalysisStatus | null | undefined) {
  ...
  switch (status) {
    ...
    case "action_required":
      return copy.statusActionRequired;
    case "failed":
...
export function mobileImageCategoryLabel(locale: Locale, category: OfflineImageType | StoreVisitImageCategory | "uncategorized" | null | undefined) {
  switch (category) {
    case "makuku_shelf":
    case "own_shelf":
      return locale === "zh" ? "Makuku 价格标签" : "Makuku Price Tags";
