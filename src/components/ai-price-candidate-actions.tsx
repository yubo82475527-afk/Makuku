...
export function AiPriceCandidateActions({
  ...
  async function deleteCandidate() {
    if (!window.confirm("Delete this incorrect price candidate?")) return;
    setLoading("delete");
    setError(null);
    try {
      const response = await fetch(`/api/ai-price-candidates/${id}`, {
  ...
  const approveDisabled = loading !== null || !previewPricePerPiece;
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        ...
        >
          {loading === "delete" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <X className="h-3.5 w-3.5" />}
          Delete
        </button>
      </div>
