...
import { Badge, Card, EmptyState, SelectInput, TextInput } from "@/components/ui";
import { formatJakartaTime, formatPercent, formatPricePerPiece } from "@/lib/format";
import type { DashboardData, DashboardSearchParams, ExecutionBoard } from "@/lib/dashboard-data";
import type {
  Alert,
...
export function DashboardContent({
  ...
  return (
    <section className="space-y-6">
      <PriceIndexSection locale={locale} board={data.priceBoard} isZh={isZh} />
      <ExceptionSection
        locale={locale}
        isZh={isZh}
        summary={data.exceptionSummary}
        battles={data.battles}
        alerts={data.alerts}
        query={query}
      />
      <ExecutionSection isZh={isZh} board={data.executionBoard} query={query} />
    </section>
  );
}
