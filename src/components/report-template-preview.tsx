...
export function ReportTemplatePreview({
  ...
  return (
    <div className="mx-auto w-full max-w-[1080px] rounded-[24px] border border-slate-200 bg-[linear-gradient(180deg,#f8fafc_0%,#f3f6fb_100%)] p-9 shadow-[0_24px_64px_rgba(15,23,42,0.08)] text-slate-800">
      <div className="sr-only">
        {isZh ? "指标说明" : "Key Metric Definitions"} Competitor Price Records
      </div>
      ...
      <div className="mb-7 flex items-start justify-between gap-8">
        <div className="grid gap-2">
          ...
          <h1 className="text-[28px] font-bold leading-[1.25] text-slate-950">
            {isZh ? "SFA-执行日报（总部）" : "SFA Execution Daily Report (HQ)"}
          </h1>
          <p className="text-[14px] leading-6 text-[#111111]">
            {isZh ? "按 Google 地图解析省份汇总昨日门店拜访与价格采集情况" : "Prior-day store visit and price capture summary grouped by Google Maps province"}
          </p>
        </div>
        ...
        <table className="w-full table-fixed border-collapse text-left">
          <thead className="bg-slate-50">
            <tr className="[&>th]:border-b [&>th]:border-slate-200 [&>th]:px-4 [&>th]:py-[18px] [&>th]:text-[14px] [&>th]:font-bold [&>th]:leading-[1.45] [&>th]:text-slate-700">
              <th className="w-[22%]">{isZh ? "省" : "Province"}</th>
              <th className="w-[16%]">{isZh ? "拜访门店数" : "Visited Stores"}</th>
              <th className="w-[14%]">{isZh ? "拜访人数" : "Visiting Employees"}</th>
              <th className="w-[24%]">{isZh ? "MAKUKU价格" : "Makuku Prices"}</th>
              <th className="w-[24%]">{isZh ? "竞品价格" : "Competitor Prices"}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, index) => (
              <tr
                ...
                className="[&>td]:border-b [&>td]:border-slate-100 [&>td]:px-4 [&>td]:py-[18px] [&>td]:align-top [&>td]:text-[15px] [&>td]:leading-[1.55] odd:bg-white even:bg-slate-50/40 last:[&>td]:border-b-0"
              >
      ...
      <div className="mt-4 flex justify-between gap-6 text-[13px] leading-5 text-slate-500">
        <div>
          <span className="font-semibold text-slate-700">{isZh ? "口径" : "Definition"}：</span>
          {isZh ? "门店与员工按去重口径统计；价格数按已采集记录统计。" : "Stores and employees are deduplicated; price counts are based on captured records."}
        </div>
        <div className="shrink-0">
          <span className="font-semibold text-slate-700">{isZh ? "范围" : "Scope"}：</span>
          {report.scope_name}
        </div>
      </div>
    </div>
