...
export default async function ExcelPriceImportPage({
  ...
  return (
    <>
      <PageShellState locale={locale} dict={dict} title={isZh ? "线下价格 Excel 导入" : "Offline Price Excel Import"} currentPath="/competitor-products" />
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-950">{isZh ? "线下价格 Excel 导入" : "Offline Price Excel Import"}</h1>
          <p className="mt-1 text-sm text-slate-500">
            {isZh
              ? "先预览，不写库；确认导入后会创建或复用门店和竞品商品，并生成门店价格快照。"
