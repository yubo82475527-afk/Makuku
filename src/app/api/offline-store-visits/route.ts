...
async function findReusableEmptyDraft(input: {
  ...
  const buildQuery = (includeUploaderUserId: boolean) => {
    let query = input.supabase
      ...
      .eq("uploader_name", input.uploaderName)
      .order("created_at", { ascending: false })
      .limit(10);
    if (includeUploaderUserId && input.uploaderUserId) {
      query = query.eq("uploader_user_id", input.uploaderUserId);
    }
    return query;
  };
...
export async function POST(request: Request) {
  try {
    ...
    if (reusableDraft) {
      ...
      return Response.json({ visit: reusableDraft });
    }
