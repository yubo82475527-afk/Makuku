...
import {
  createPriceIndexExportSignedUrl,
  getPriceIndexExportDownloadName,
...
export async function GET(request: Request, ctx: { params: Promise<{ jobId: string }> }) {
  ...
  try {
    ...
    const signedUrl = await createPriceIndexExportSignedUrl({
      filePath: job.file_path,
      downloadName: getPriceIndexExportDownloadName(job),
    });
