...
import {
  failPriceSnapshotExportJob,
  runPriceSnapshotExportJob,
...
async function runAndRespond(jobId: string) {
  try {
    return Response.json(await runPriceSnapshotExportJob({ jobId }));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown error";
    try {
      await failPriceSnapshotExportJob({ jobId, message });
    } catch {}
